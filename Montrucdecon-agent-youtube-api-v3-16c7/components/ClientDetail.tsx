...
const GeminiStrategicAnalysis: React.FC<{client: Client, projects: Project[], notes: Note[]}> = ({ client, projects, notes }) => {
    ...
    const handleAiAnalysis = async () => {
        ...
        // CORRECTION: Remplacer l'appel direct (qui exposait la clé API)
        // par un appel fetch à la fonction Netlify /api/gemini-generate
        try {
            const response = await fetch('/api/gemini-generate', {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({ 
                    prompt: prompt,
                    model: 'gemini-2.5-pro' // On spécifie le modèle Pro ici
                }),
            });
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || 'Failed to generate strategic analysis');
            }
