...
const NewEventModal: React.FC<NewEventModalProps> = ({ onClose, onTaskCreated, payload }) => {
  const { projects, clients, addTask } = useAppContext();
  const [text, setText] = useState('');
  const [projectId, setProjectId] = useState('');
  const [priority, setPriority] = useState<TaskPriority>('Moyenne');
  const [dueDate, setDueDate] = useState(payload?.date || new Date().toISOString().split('T')[0]);
  ...
  return (
    <ModalWrapper title="Nouvel Événement" onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium mb-1 text-neutral-600 dark:text-neutral-300">Tâche / Événement *</label>
          <input type="text" value={text} onChange={e => setText(e.target.value)} placeholder="Ex: Call de suivi avec Sophie" required className="w-full p-2 bg-neutral-100 dark:bg-neutral-700 border border-neutral-300 dark:border-neutral-600 rounded-lg focus:ring-primary-500 focus:border-primary-500"/>
        </div>
        <div>
